/* SPDX-License-Identifier: LGPL-3.0-or-later */
#include <assert.h>
#include <stdio.h>
#include <stdlib.h>
#include "mcb/func.h"
#include "mcb/inst.h"
#include "mcb/inst/element_of.h"
#include "mcb/value.h"

#define LIBMCB_STRIP
#include "gen_mov.h"
#include "gnu_asm.h"
#include "inst.h"
#include "mem.h"
#include "reg.h"
#include "struct.h"
#include "utils.h"
#include "value.h"

#include "../../ealloc.h"
#include "../../err.h"
#include "../../str.h"
#include "../../text_block.h"

static enum GNU_ASM_REG mov_idx_to_reg(
		struct gnu_asm_value *container,
		const struct gnu_asm_value *idx,
		struct mcb_func *fn,
		struct gnu_asm *ctx);

enum GNU_ASM_REG
mov_idx_to_reg(
		struct gnu_asm_value *container,
		const struct gnu_asm_value *idx,
		struct mcb_func *fn,
		struct gnu_asm *ctx)
{
	struct text_block *blk;
	struct gnu_asm_value fake_idx, src;
	enum GNU_ASM_REG result;

	assert(container && idx && fn && ctx);

	if (IS_REG(idx->kind))
		return idx->inner.reg;

	src = *idx;
	fake_idx.kind = remap_value_kind(I8_REG_VALUE, src.kind);
	fake_idx.inner.reg = alloc_reg(AUTO_ALLOC_REG, container, fn);
	if (fake_idx.inner.reg == REG_COUNT) {
		if (mov_reg_user(AUTO_ALLOC_REG - 1, fn, ctx))
			eabort("mov_reg_user()");
		drop_reg(AUTO_ALLOC_REG - 1, fn);
		fake_idx.inner.reg = alloc_reg(AUTO_ALLOC_REG - 1, container, fn);
		if (fake_idx.inner.reg == REG_COUNT)
			eabort("alloc_reg()");
	}

	estr_clean(&ctx->buf);
	if (gen_mov(&ctx->buf, &fake_idx, &src, fn, ctx))
		eabort("gen_mov()");
	blk = text_block_from_str(&ctx->buf);
	append_text_block(&ctx->text, blk);

	result = fake_idx.inner.reg;

	return result;
}

int
build_element_of_inst(struct mcb_inst *inst_outer,
		struct mcb_func *fn,
		struct gnu_asm *ctx)
{
	const struct gnu_asm_value *container;
	struct mcb_element_of_inst *inst;
	struct gnu_asm_mem_obj *mem;
	struct gnu_asm_value *result;
	assert(inst_outer && fn && ctx);
	inst = &inst_outer->inner.element_of;
	assert(inst);

	assert(inst->result->data == NULL);
	assert(inst->container->data);

	container = inst->container->data;

	result = ecalloc(1, sizeof(*result));
	result->container = inst->result;
	result->kind = map_type_to_value_kind(
			I8_MEM_VALUE,
			inst->result->type);

	assert(IS_MEM(container->kind));
	mem = ecalloc(1, sizeof(*mem));
	mem->base = container->inner.mem->base;
	mem->kind = VAR_MEM;
	mem->offset = container->inner.mem->offset;
	mem->index = mov_idx_to_reg(result, inst->idx->data, fn, ctx);
	mem->scale = map_value_kind_to_bytes(result->kind);
	mem->user = result;

	result->inner.mem = mem;
	inst->result->data = result;

	if (inst->idx->scope_end == inst_outer)
		drop_value(inst->idx, fn);
	
	return 0;
}
